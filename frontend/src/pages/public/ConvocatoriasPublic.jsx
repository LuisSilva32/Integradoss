...
export default function ConvocatoriasPublic () {
  const { information, loading, error, fetchInformation } = useInformation('convocatoria')
  ...
  return (
    <>
      <Helmet>
        <title>Convocatorias - Integradoss</title>
      </Helmet>
      <div className='min-h-screen flex flex-col'>
        ...
        <main className='flex-grow pt-24 container mx-auto px-12 md:px-16'>
          <div>
            ...
            >
              <h1 className='text-2xl md:text-4xl font-light text-gray-800 mb-4'>Convocatorias - Integradoss</h1>
              <p className='text-sm text-gray-600 text-justify md:text-center md:text-md max-w-2xl mx-auto'>
                Explora las convocatorias disponibles del semillero Integradoss y encuentra oportunidades para participar en proyectos de investigación.
              </p>
            </motion.div>
            ...
            {information.length === 0
              ? (
                <div className='text-center text-gray-600 py-12'>
                  <p>No hay convocatorias disponibles actualmente.</p>
                </div>
                )
              : loading
                ? (
        ...
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          <DialogContent className='max-w-2xl'>
            <DialogHeader>
              <DialogTitle className='ml-2'>{selectedCall?.title}</DialogTitle>
            </DialogHeader>
            {selectedCall && (
              <div className='mt-4 space-y-4'>
                {selectedCall.image && (
                  <div className='w-full h-48 border border-gray-400 overflow-hidden p-1.5'>
                    <img
                      src={selectedCall.image || '/placeholder.svg'}
                ...
                <ScrollArea className='h-64 pr-4'>
                  <div className='whitespace-pre-line text-justify text-gray-600'>{selectedCall.description}</div>
                </ScrollArea>
              </div>
            )}
          </DialogContent>
