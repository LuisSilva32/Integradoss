...
export default function InvestigadoresPublic () {
  const { researchers, loading, error, fetchResearchers } = useResearcher()
  const [selectedResearcher, setSelectedResearcher] = useState(null)
  const [openDialog, setOpenDialog] = useState(false)

  useEffect(() => {
    fetchResearchers(null) // Sin token para acceso público
  }, [fetchResearchers])
  ...
  return (
    <>
      <Helmet>
        <title>Investigadores - Integradoss</title>
      </Helmet>
      <div className='min-h-screen flex flex-col'>
        ...
        <main className='flex-grow pt-24 container mx-auto px-12 md:px-16'>
          <div>
            ...
            >
              <h1 className='text-2xl md:text-4xl font-light text-gray-800 mb-4'>Investigadores - Integradoss</h1>
              <p className='text-sm text-gray-600 text-justify md:text-center md:text-md max-w-2xl mx-auto'>
                Conoce a los investigadores que forman parte del semillero Integradoss y sus contribuciones.
              </p>
            </motion.div>
            ...
            {researchers.length === 0
              ? (
                <div className='text-center text-gray-600 py-12'>
                  <p>No hay investigadores disponibles actualmente.</p>
                </div>
                )
              : loading
                ? (
        ...
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
          <DialogContent className='max-w-2xl'>
            <DialogHeader>
              <DialogTitle className='ml-2'>{selectedResearcher?.name}</DialogTitle>
            </DialogHeader>
            {selectedResearcher && (
              <div className='mt-4 space-y-4'>
                ...
                <ScrollArea className='h-64 pr-4'>
                  <div className='whitespace-pre-line text-justify text-gray-600'>{selectedResearcher.description}</div>
                </ScrollArea>
              </div>
            )}
          </DialogContent>
